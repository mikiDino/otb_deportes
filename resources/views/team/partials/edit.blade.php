<form method="POST" action="{{ route('equipos.update', $team->id) }}">
    @csrf
    @method('PUT')
    <div class="form-group">
        <label for="nombre_equipo">Nombre del Equipo</label>
        <input id="nombre_equipo" type="text" class="form-control @error('nombre_equipo') is-invalid @enderror"
            name="nombre_equipo" value="{{ $team->nombre }}" required autofocus>
        @error('nombre_equipo')
    ...
    <div class="form-group">
        ...
        <input id="nombre_barrio" type="text" class="form-control @error('nombre_barrio') is-invalid @enderror"
            name="nombre_barrio" value="{{ $team->ciudad }}" required autofocus>
        @error('nombre_barrio')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
    ...
    <div class="form-group">
        ...
        <input id="nombre_dt" type="text" class="form-control @error('nombre_dt') is-invalid @enderror"
            name="nombre_dt" value="{{ $team->nombre_DT }}" required>
        @error('nombre_dt')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
    ...
    <div class="form-group">
        ...
        <input id="celular_dt" type="text" class="form-control @error('celular_dt') is-invalid @enderror"
            name="celular_dt" value="{{ $team->celular_DT }}" required>
        @error('celular_dt')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
    ...
    <div class="form-group">
        ...
        <select id="categoria_id" class="form-control" name="categoria_id" required>
            @foreach ($categorias as $categoria)
                <option value="{{ $categoria->id }}" {{ $team->categoria_id == $categoria->id ? 'selected' : '' }}>
                    {{ $categoria->nombre }}
                </option>
            @endforeach
        </select>
        ...
        <div class="scroll-container">
            <ul id="lista-jugadores" class="list-group">
                @foreach ($team->jugadores as $jugador)
                    <li class="list-group-item">{{ $jugador->nombre }} {{ $jugador->apellidos }} ({{ $jugador->edad }}
                        años)
                        <i class="fas fa-times-circle btn btn-outline-danger rounded-circle btn-remove"></i>
                    </li>
                @endforeach
            </ul>
        </div>
